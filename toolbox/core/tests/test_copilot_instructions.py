...
def test_copilot_instructions_file_exists():
    ...
    project_root = Path(__file__).parents[
        3
    ]  # Go up from toolbox/core/tests to project root
...
def test_copilot_instructions_file_content():
    ...
    for section in expected_sections:
        assert (
            section in content
        ), f"Expected section '{section}' not found in copilot instructions"
...
def test_copilot_instructions_file_size():
    ...
    assert (
        file_size > 1000
    ), "Copilot instructions file should contain substantial content"
    assert (
        file_size < 50000
    ), "Copilot instructions file should not be excessively large"
